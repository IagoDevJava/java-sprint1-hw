...
ТЗ: «Счётчик калорий». Этот проект как раз для меня!
...
Как хранить данные
По условию задачи нужно учитывать шаги каждый месяц. Другими словами, требуется по указанному месяцу получить какие-то данные, которые относятся к этому месяцу. Один из способов это сделать — воспользоваться классом HashMap для связывания месяца и данных по нему. Создать экземпляр HashMap можно прямо в конструкторе StepTracker. В качестве ключа подойдёт к примеру порядковый номер месяца. В таком случае 0 будет обозначать январь, а 6 — июль, так как счёт начинается с нуля, а не с единицы.
